...
const Invordering_Banken = require('../models/invordering_banken');
const dateoptions = { year: 'numeric', month: 'short' };


exports.invordering_list = (req, res, next) => {
    Invordering_Banken.find({$or: [{ "_base_record": "false" }, { "_baseline": "true" }]})
        .exec()
        .then(docs => {
            const response = {
                count: docs.length,
...
exports.invordering_list_base_records = (req, res, next) => {
    Invordering_Banken.find({$or: [{ "_base_record": "true" }, { "_baseline": "true" }]})
        .exec()
        .then(docs => {
            const response = {
                count: docs.length,
...
exports.invordering_find_one = (req, res, next) => {
    ...
    Invordering_Banken.find({ _id: { $eq: id } })
        .exec()
        .then(docs => {
            if (docs.length >= 1) {
                const response = {
...
exports.invordering_create_one = (req, res, next) => {
    Invordering_Banken.insertMany(req.body)
        .then(result => {
            console.log(result);
            res.status(201).json({
...
exports.invordering_update_one = (req, res, next) => {
    const id = req.params.invorderingId;
    Invordering_Banken.updateMany({ _id: { $eq: id } }, { $set: req.body }, { upsert: true })
        .exec()
        .then(result => {
            console.log(result)
            res.status(200).json({
...
exports.invordering_delete_one = (req, res, next) => {
    const id = req.params.invorderingId;
    Invordering_Banken.deleteOne({ _id: { $eq: id } })
        .exec()
        .then(result => {
...
exports.invordering_delete_non_base_records = (req, res, next) => {
    Invordering_Banken.remove({ "_base_record": "false" })
        .exec()
        .then(result => {
            if (result.deletedCount !== 0) {
                console.log(result)
